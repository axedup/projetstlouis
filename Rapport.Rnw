...
Délai médian rechute : \Sexpr{median(difftime(greffe$date_rechutep,greffe$j0),na.rm=TRUE)} jours.
...
\subsection{OS et PFS après une cgvhd}
...
\subsection{Survival analysis in front line patients n=\Sexpr{length(greffe$greffe.en..front.line..1...en.2e.ligne.ou.plus...cad.après.rechute..0==1)}}
