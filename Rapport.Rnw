...
%\pagebreak
%EFS at 1 year was \Sexpr{round(reefs$surv[which.min((reefs$time-12)<0)],2)} (95 \% \Sexpr{round(reefs$lower[which.min((reefs$time-12)<0)-1],2)} - \Sexpr{round(reefs$upper[which.min((reefs$time-12)<0)-1],2)}), was \Sexpr{round(reefs$surv[which.min((reefs$time-24)<0)-1],2)} (95 \% \Sexpr{round(reefs$lower
%[which.min((reefs$time-24)<0)-1],2)} - \Sexpr{round(reefs$upper[which.min((reefs$time-24)<0)-1],2)}) at 2 years. EFS at 4 years was \Sexpr{round(reefs$surv[which.min((reefs$time-48)<0)-1],2)} (95 \% \Sexpr{round(reefs$lower[which.min((reefs$time-48)<0)-1],2)} - \Sexpr{round(reefs$upper[which.min((reefs$time-48)<0)-1],2)}).

%\begin{figure}[h]
%\begin{center}
<<label=fig2,fig=TRUE,echo=FALSE,eval=FALSE>>=
efs_km
@
%\end{center}
...
\captionof{figure}{CIF of GVHD and Death without GVHD}
...
\captionof{figure}{CIF of GVHD and Death without GVHD (in patients with a complete remission post alloSCT)}
