...
tableau<-cbind(rep(i,4),c("os","efs","trm","cif rechute/progression"),tableau)
...
colnames(TABLEAU)<-c("histo","outcome","1 an","BI 1 an","BS 1 an",
...
for (i in c("1")) {
  ...
  trash<-greffe[greffe$greffe.en..front.line..1...en.2e.ligne.ou.plus...cad.après.rechute..0 %in% i,]
  summary(as.numeric(trash$delai_dc))
  s<-Surv(event=trash$deces,time=as.numeric(trash$delai_dc))
...
km_trmt<-ggplot()+ geom_step(data=evenementtrm,aes(x=time, y=1-ev),color="black", direction="hv")  +
  ...
  TABLEAUF<-rbind(TABLEAUF,tableau)
...
colnames(TABLEAUF)<-c("histo","outcome","1 an","BI 1 an","BS 1 an",
                     "2 ans","BI 2 an","BS 2 an","4 an","BI 4 an","BS 4 an")
